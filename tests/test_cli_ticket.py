...
class TestTicketListClosedCount:
    ...
    def test_status_filter_no_closed_hint(self) -> None:
    ...
    def test_blocked_filter_no_closed_hint(self) -> None:
        ...
        with runner.isolated_filesystem():
            ...
            result = runner.invoke(cli.app, ["tk", "list", "--blocked"])

            assert result.exit_code == 0
            assert "No open tickets" not in result.output
            assert "No tickets found" in result.output

    def test_ready_filter_no_closed_hint(self) -> None:
        ...
        with runner.isolated_filesystem():
            ...
            result = runner.invoke(cli.app, ["tk", "list", "--ready"])
...
class TestTicketDep:
    ...
    def test_dep_appends_not_overwrites(self) -> None:
        ...
        with runner.isolated_filesystem():
            ...
            result1 = runner.invoke(cli.app, ["tk", "deps", "add", "cf1a", "3642"])
            assert result1.exit_code == 0, result1.output
            assert "now depends on" in result1.output

            # Second dep: cf1a depends on d869
            result2 = runner.invoke(cli.app, ["tk", "deps", "add", "cf1a", "d869"])
    ...
    def test_dep_preserves_existing_deps_on_disk(self) -> None:
        ...
        with runner.isolated_filesystem():
            ...
            result = runner.invoke(cli.app, ["tk", "deps", "add", "cf1a", "d869"])
            assert result.exit_code == 0, result.output
    ...
    def test_dep_survives_status_change(self) -> None:
        ...
        with runner.isolated_filesystem():
            ...
            # Add first dep
            runner.invoke(cli.app, ["tk", "deps", "add", "cf1a", "aaaa"])
            # Change status (rewrites ticket)
            runner.invoke(cli.app, ["tk", "start", "cf1a"])
            # Add second dep
            runner.invoke(cli.app, ["tk", "deps", "add", "cf1a", "bbbb"])
    ...
    def test_dep_idempotent(self) -> None:
        ...
        with runner.isolated_filesystem():
            ...
            # Add dep twice
            runner.invoke(cli.app, ["tk", "deps", "add", "cf1a", "aaaa"])
            result = runner.invoke(cli.app, ["tk", "deps", "add", "cf1a", "aaaa"])
    ...
    def test_dep_not_found(self) -> None:
        ...
        with runner.isolated_filesystem():
            ...
            result = runner.invoke(cli.app, ["tk", "deps", "add", "cf1a", "zzzz"])
            assert result.exit_code == 1
            assert "not found" in result.output
...
class TestTicketUndep:
    ...
    def test_undep_removes_dependency(self) -> None:
        with runner.isolated_filesystem():
            ...
            result = runner.invoke(cli.app, ["tk", "deps", "remove", "cf1a", "aaaa"])
    ...
    def test_undep_not_a_dep(self) -> None:
        with runner.isolated_filesystem():
            ...
            result = runner.invoke(cli.app, ["tk", "deps", "remove", "cf1a", "zzzz"])
    ...
    def test_undep_partial_match(self) -> None:
        with runner.isolated_filesystem():
            ...
            result = runner.invoke(cli.app, ["tk", "deps", "remove", "cf1a", "abcd"])
    ...
    def test_undep_resolves_via_find_ticket(self) -> None:
        ...
        with runner.isolated_filesystem():
            ...
            result = runner.invoke(cli.app, ["tk", "deps", "remove", "cc03", "ab"])
    ...
    def test_undep_preserves_other_deps(self) -> None:
        with runner.isolated_filesystem():
            ...
            runner.invoke(cli.app, ["tk", "deps", "add", "aaaa", "bbbb"])
            runner.invoke(cli.app, ["tk", "deps", "add", "aaaa", "cccc"])
            runner.invoke(cli.app, ["tk", "deps", "remove", "aaaa", "bbbb"])
...
class TestNoResultsMessages:
    ...
    def test_ready_empty_shows_no_tickets(self) -> None:
        with runner.isolated_filesystem():
            base = Path.cwd()
            setup_project(base)
    ...
    def test_ready_filters_with_all(self) -> None:
        with runner.isolated_filesystem():
            base = Path.cwd()
            setup_project(base)
            tickets_dir = branch_root(base, BRANCH) / "tickets"
            ...
            result = runner.invoke(cli.app, ["tk", "list", "--ready", "--all"])

            assert result.exit_code == 0
            assert "aaaa" in result.output
            assert "bbbb" in result.output

    def test_ready_excludes_blocked_tickets(self) -> None:
        with runner.isolated_filesystem():
            base = Path.cwd()
            setup_project(base)
            tickets_dir = branch_root(base, BRANCH) / "tickets"
            ...
            blocked = Ticket(
                id="bbbb", status="open", title="Blocked", body="", deps=["aaaa"], created=datetime.now(UTC)
            )
...
class TestTicketLink:
    ...
    def test_link_creates_symmetric_links(self) -> None:
        with runner.isolated_filesystem():
            base = Path.cwd()
            setup_project(base)
            tickets_dir = branch_root(base, BRANCH) / "tickets"
    ...
    def test_unlink_removes_symmetric_links(self) -> None:
        with runner.isolated_filesystem():
            base = Path.cwd()
            setup_project(base)
            tickets_dir = branch_root(base, BRANCH) / "tickets"
    ...
    def test_link_needs_at_least_two(self) -> None:
        with runner.isolated_filesystem():
            base = Path.cwd()
            setup_project(base)
...
class TestTicketDepTree:
    ...
    def test_tree_shows_deps(self) -> None:
        with runner.isolated_filesystem():
            base = Path.cwd()
            setup_project(base)
            tickets_dir = branch_root(base, BRANCH) / "tickets"
            ...
            result = runner.invoke(cli.app, ["tk", "deps", "tree", "aaaa"])
            assert result.exit_code == 0
            assert "Root" in result.output
            assert "bbbb" in result.output
            assert "Dep" in result.output

    def test_tree_deduplicates_by_default(self) -> None:
        with runner.isolated_filesystem():
            base = Path.cwd()
            setup_project(base)
            tickets_dir = branch_root(base, BRANCH) / "tickets"
            write_ticket(
                Ticket(
                    id="aaaa", status="open", title="Root", body="", deps=["bbbb", "cccc"], created=datetime.now(UTC)
                ),
                tickets_dir / "aaaa.md",
            )
            write_ticket(
    ...
    def test_tree_full_with_cycle_does_not_recurse_infinitely(self) -> None:
        ...
        with runner.isolated_filesystem():
            ...
            result = runner.invoke(cli.app, ["tk", "deps", "tree", "--full", "aaaa"])
            assert result.exit_code == 0
...
class TestTicketDepCycle:
    ...
    def test_no_cycles(self) -> None:
        with runner.isolated_filesystem():
            ...
            result = runner.invoke(cli.app, ["tk", "deps", "cycle"])
            assert result.exit_code == 0
...
class TestTicketBlocked:
    ...
    def test_lists_blocked_tickets(self) -> None:
        with runner.isolated_filesystem():
            base = Path.cwd()
            setup_project(base)
            tickets_dir = branch_root(base, BRANCH) / "tickets"
...
class TestTicketClosed:
    ...
    def test_lists_closed_tickets(self) -> None:
        with runner.isolated_filesystem():
            base = Path.cwd()
            setup_project(base)
            tickets_dir = branch_root(base, BRANCH) / "tickets"
    ...
    def test_closed_flag_includes_closed_with_open(self) -> None:
        with runner.isolated_filesystem():
            base = Path.cwd()
            setup_project(base)
            tickets_dir = branch_root(base, BRANCH) / "tickets"
    ...
    def test_status_closed_filter(self) -> None:
...
class TestTicketAddNote:
    ...
    def test_adds_note_to_ticket(self) -> None:
        with runner.isolated_filesystem():
            base = Path.cwd()
            setup_project(base)
            tickets_dir = branch_root(base, BRANCH) / "tickets"
...
class TestTicketListJson:
    ...
    def test_json_excludes_closed_by_default(self) -> None:
        import json
    ...
    def test_json_closed_includes_all(self) -> None:
        import json
        ...
        with runner.isolated_filesystem():
            ...
            result = runner.invoke(cli.app, ["tk", "list", "--json", "--closed"])
            assert result.exit_code == 0
            data = json.loads(result.output)
            assert len(data) == 2
...
class TestTicketListFilters:
    ...
    def test_filter_by_assignee(self) -> None:
        with runner.isolated_filesystem():
            ...
            result = runner.invoke(cli.app, ["tk", "list", "--assignee", "alice"])
            assert result.exit_code == 0
            assert "aaaa" in result.output
            assert "bbbb" not in result.output
    ...
    def test_filter_by_tag(self) -> None:
        with runner.isolated_filesystem():
            ...
            result = runner.invoke(cli.app, ["tk", "list", "--tag", "frontend"])
            assert result.exit_code == 0
            assert "aaaa" in result.output
            assert "bbbb" not in result.output
...
class TestTicketShowRelationships:
    ...
    def test_show_displays_blocking(self) -> None:
        with runner.isolated_filesystem():
            base = Path.cwd()
            setup_project(base)
            tickets_dir = branch_root(base, BRANCH) / "tickets"
            ...
            result = runner.invoke(cli.app, ["tk", "show", "aaaa"])
            assert result.exit_code == 0
            assert "Blocking" in result.output
            assert "bbbb" in result.output

    def test_show_displays_children(self) -> None:
        with runner.isolated_filesystem():
            base = Path.cwd()
            setup_project(base)
            tickets_dir = branch_root(base, BRANCH) / "tickets"
