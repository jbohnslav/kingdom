...
Contract refresh: 2026-09-16 for Kingdom 1.1.0.

Current CLI observations: Claude Code 2.1.273; Codex CLI 0.154.0.
`cursor-agent` is not installed in this environment, so Cursor evidence remains
Contract. The historical Live observations below are from 2026-08-21, Kingdom
1.0.0, baseline `a9120675c2cadcd62c9117f8e2cc329f48cdd7d9`; they have not been
rerun for this release.

For 1.1.0, Stop emits at most one advisory `systemMessage` per turn for Claude and
Codex when work was observed without a ticket update. It does not block or start
another turn. This is a host UI notice; the prompt reminder supplies model
context. Cursor Stop is silent because its `followup_message` forces continuation.
Exact-ticket isolation and compaction/handoff contracts remain covered by tests.
