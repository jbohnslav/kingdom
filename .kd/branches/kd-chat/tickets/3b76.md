...
- [ ] **Rename `auto_rounds` → `auto_messages`** in config, Council, validation
- [ ] **Budget is messages, not rounds** — single counter loop in `run_chat_round`
- [ ] **First king message: no auto-turns** — infer from thread history
- [ ] **Follow-up king messages: auto-turns up to N messages** (default len(members))
