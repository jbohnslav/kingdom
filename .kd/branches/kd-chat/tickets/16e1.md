...
priority: 1
---
# User input, query dispatch, and slash commands

## Context

Handle user input: Enter sends, Shift+Enter newlines, @member addressing, slash commands. On send, write king message to thread files, then launch `member.query()` via `asyncio.to_thread()`. Uses `Council.create()` directly — no CLI shelling.
...
- [ ] `/mute <member>` excludes member from future queries (session-scoped)
- [ ] `/unmute <member>` re-includes member
