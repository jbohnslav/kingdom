...
- [ ] `watch_thread()` tails `.stream-{member}.jsonl` files and displays incremental text during execution (extract `text_delta` text, show accumulated text per member, replace with final response when message file lands)
